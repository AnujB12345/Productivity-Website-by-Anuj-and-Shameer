...
def timer(request):
    ...
    settings_obj, _ = PomodoroSettings.objects.get_or_create(username=username)
    ...
    if request.method == "POST":
        ...
        if "log_session" in request.POST:
            ...
            # returns JSON rather than redirecting, since the page must NOT
            # reload (that would reset the running timer's JS state).
        ...
        if "delete_session" in request.POST:
            ...
            PomodoroSession.objects.filter(id=session_id, username=username).delete()
            return redirect("timer:timer")

    today_start = timezone.now().replace(hour=0, minute=0, second=0, microsecond=0)
    todays_work_sessions = PomodoroSession.objects.filter(
        username=username, session_type="work", completed_at__gte=today_start
    )
    sessions_today = todays_work_sessions.count()
    focus_minutes_today = sum(s.duration_minutes for s in todays_work_sessions)

    recent_sessions = PomodoroSession.objects.filter(username=username)[:10]
    ...
    return render(request, "timer_page.html", {
        ...
        "settings_dict": settings_dict,   # was settings_json
