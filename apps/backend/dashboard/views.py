...
def dashboard(request):
    ...
    if request.method == "POST" and ("update_goal" in request.POST or "update_pomodoro_goal" in request.POST):
        goal_obj, _ = Goal.objects.get_or_create(user=current_user)
    ...
    tasks_this_week = Todo.objects.filter(user=current_user, completed_at__gte=week_0_start).count()
    tasks_last_week = Todo.objects.filter(
        user=current_user, completed_at__gte=week_1_start, completed_at__lt=week_0_start
    ).count()

    notes_this_week = Note.objects.filter(user=current_user, created_at__gte=week_0_start).count()
    ...
    events_done_last_month = CalendarEvent.objects.filter(
        user=current_user, date__gte=month_ago, date__lt=today
    ).count()
    events_upcoming = CalendarEvent.objects.filter(
        user=current_user, date__gte=today, date__lte=month_ahead
    ).count()
    ...
    tasks_completed_this_month = Todo.objects.filter(
        user=current_user, checked=True, completed_at__gte=month_start
    ).count()
    tasks_progress_pct = round((tasks_completed_this_month / total_tasks) * 100) if total_tasks else 0


    # --- Notes and events progress tracker ---
    goal_obj, _ = Goal.objects.get_or_create(user=current_user)
    notes_created_this_month = Note.objects.filter(user=current_user, created_at__gte=month_start).count()
    ...
    events_this_month_total = CalendarEvent.objects.filter(
        user=current_user, date__year=today.year, date__month=today.month
    ).count()
    events_this_month_done = CalendarEvent.objects.filter(
        user=current_user, date__year=today.year, date__month=today.month, date__lt=today
    ).count()
